...
class Event(models.Model):
    ...
    def total_tickets_sold(self):
        ...
        return (
            Ticket.objects.filter(tier__event=self, status="SOLD").aggregate(
                total=Sum("quantity")
            )["total"]
            or 0
        )
...
class Ticket(models.Model):
    ...
    cash_verification_code = models.CharField(max_length=8, blank=True, null=True, unique=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if (self.status == "SOLD" and not self.qr_code) or (self.payment_method == "CASH" and self.cash_payment_verified):
