...
class EventForm(forms.ModelForm):
    class Meta:
        model = Event
        fields = ('title', 'description', 'category', 'venue', 'date', 'time', 
                 'capacity', 'image')
        widgets = {
            ...
            'capacity': forms.NumberInput(attrs={'min': '1', 'placeholder': 'Enter maximum capacity'}),
        }
        help_texts = {
...
class TicketTierForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'description': forms.Textarea(attrs={'rows': 4, 'placeholder': 'Describe what this ticket tier includes'}),
            'price': forms.NumberInput(attrs={'min': '0', 'step': '0.01', 'placeholder': 'Enter ticket price'}),
            'quantity': forms.NumberInput(attrs={'min': '1', 'placeholder': 'Enter number of tickets available'}),
        }
        help_texts = {
